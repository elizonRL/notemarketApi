...
export default [
  // 1. Aplicamos las reglas básicas recomendadas de JavaScript
  ...
  // 2. Definimos nuestro entorno Node.js y las reglas de estilo de Standard
  {
    ...
    rules: {
      ...
      'quotes': ['error', 'single', {              // Obligatorio: Comillas simples ('')
        'avoidEscape': true,
        'allowTemplateLiterals': true
      }],
      'indent': ['error', 2, {                     // Obligatorio: Sangría de 2 espacios
        'SwitchCase': 1
      }],
      'comma-spacing': ['error', {                 // Espacio después de las comas (a, b)
        'before': false,
        'after': true
      }],
      'space-before-blocks': ['error', 'always'],  // Espacio antes de abrir llaves: if () {}
      'space-before-function-paren': ['error', {   // Espacio en funciones: function () {}
        'anonymous': 'always',
        'named': 'never',
        'asyncArrow': 'always'
      }],
      'keyword-spacing': ['error', {               // Espacio alrededor de palabras clave (if, else, return)
        'before': true,
        'after': true
      }],
      'key-spacing': ['error', {                   // Espacio en objetos { clave: valor }
        'beforeColon': false,
        'afterColon': true
      }],
      ...
      // --- LOGICA Y SINTAXIS ---
      'eqeqeq': ['error', 'always', {              // Obliga a usar === y !== en lugar de == y !=
        'null': 'ignore'
      }],
      'no-unused-vars': ['warn', {                 // Advierte variables no usadas (ignora las que empiezan con _)
        'argsIgnorePattern': '^_'
      }],
